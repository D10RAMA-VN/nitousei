(function (c, a, b) {
  a.kag.tag.backlog = {
    ...
    start: function (f) {
      ...
      if (f.bgimage != "") {
        d.css({
          background: "url('" + a.kag.tmp.backlog.image_path + f.bgimage + "') no-repeat"
        })
      } else {
        var e = hex2rgb(c.convertColor(f.bgcolor));
        d.css({
          "background-color": "rgba(" + e[0] + "," + e[1] + "," + e[2] + "," + c.convertOpacity(f.opacity) + ")"
        })
      }
  ...
  a.kag.tag.backlog_show = {
    ...
    start: function (e) {
      ...
      var g = c("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_chara");
      var f = c("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_text");
      var d = c("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_voice");
      g.css({
      ...
      g.each(function () {
        if (c(this).text() != "" && c(this).text().indexOf(a.kag.tmp.backlog.select_text) == -1) {
          if (a.kag.tmp.backlog.chara_pm.bgimage != "") {
            c(this).css({
              background: "url('" + a.kag.tmp.backlog.image_path + a.kag.tmp.backlog.chara_pm.bgimage + "')",
              opacity: "1"
            })
          } else {
            var h = hex2rgb(c.convertColor(a.kag.tmp.backlog.chara_pm.bgcolor));
            c(this).css({
              "background-color": "rgba(" + h[0] + "," + h[1] + "," + h[2] + "," + c.convertOpacity(a.kag.tmp.backlog.chara_pm.opacity) + ")"
            })
          }
        }
      });
  ...
  }(["backlog", "endbacklog", "backlog_area", "backlog_chara", "backlog_text", "backlog_line", "backlog_select", "backlog_button_voice", "backlog_button_close", "backlog_show", "backlog_hide", "backlog_clear", "backlog_scroll"]))
