function hex2rgb(a) {
  ...
  if (a.length == 3) {
    a = a.slice(0, 1) + a.slice(0, 1) + a.slice(1, 2) + a.slice(1, 2) + a.slice(2, 3) + a.slice(2, 3)
  }
...
TYRANO.kag.tmp.backlog.functions.addBacklog = function (a, b) {
  if (TYRANO.kag.stat.log_write) {
    if (TYRANO.kag.stat.f.backlog.length >= parseInt(TYRANO.kag.config.maxBackLogNum)) {
...
TYRANO.kag.tmp.backlog.functions.buttonHover = function (c, b) {
  ...
  a.find(".base").show();
  a.hover(function () {
    $(this).find(".base_enter").fadeIn(parseInt(TYRANO.kag.tmp.backlog.button_hover_time), function () {
      $(this).find(".base").hide()
    })
  }, function () {
    $(this).find(".base").show();
...
TYRANO.kag.tmp.backlog.functions.voiceStopButtonHover = function (b) {
  ...
  a.find(".close").show();
  a.hover(function () {
    $(this).find(".close_enter").fadeIn(parseInt(TYRANO.kag.tmp.wrapmenu.button_hover_time), function () {
...
TYRANO.kag.tmp.backlog.functions.createButton = function (f, c) {
  var a = f;
  a.append("<img class='base button' src='" + TYRANO.kag.tmp.backlog.image_path + c.img + "' style='position:absolute;'>");
  var b = a.find(".base");
  b.css({
    width: c.width + "px",
    height: c.height + "px"
  });
  a.append("<img class='base_enter button' src='" + TYRANO.kag.tmp.backlog.image_path + c.img + "' style='position:absolute;display:none;'>");
  ...
  if (typeof c.img_close !== "undefined") {
    a.append("<img class='close button' src='" + TYRANO.kag.tmp.backlog.image_path + c.img_close + "' style='position:absolute;display:none;'>");
    var e = a.find(".close");
    e.css({
      width: c.width + "px",
      height: c.height + "px"
    });
    a.append("<img class='close_enter button' src='" + TYRANO.kag.tmp.backlog.image_path + c.img_close + "' style='position:absolute;display:none;'>");
...
TYRANO.kag.tmp.backlog.functions.checkClick = function () {
  if (TYRANO.kag.stat.is_adding_text || TYRANO.kag.stat.is_skip || TYRANO.kag.stat.is_auto) {
    return false
  }
  return true
};
TYRANO.kag.tmp.backlog.functions.createLog = function () {
  ...
  for (var d = 0; d < TYRANO.kag.stat.f.backlog.length; d++) {
    ...
    a += "<div class='backlog_voice'><div data-voice='" + TYRANO.kag.stat.f.backlog[d][e.VOICE] + "'></div></div>";
    a += "<div class='backlog_chara'>" + TYRANO.kag.stat.f.backlog[d][e.CHARA] + "</div>";
    a += "<div class='backlog_text'>" + TYRANO.kag.stat.f.backlog[d][e.TEXT] + "</div>";
    a += "</div>";
...
TYRANO.kag.tmp.backlog.functions.setCharaName = function () {
  ...
  var a = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_chara");
  var c = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_text");
  a.css({
    "font-family": b.face,
    "font-size": b.size + "px",
  ...
  c.css({
    display: "table-cell",
    width: "calc(100% - " + b.width + "px)"
  })
};
TYRANO.kag.tmp.backlog.functions.setText = function () {
  var b = TYRANO.kag.tmp.backlog.text_pm;
  TYRANO.kag.tmp.backlog.text_pm.line_height = (parseInt(b.size) + parseInt(b.line));
  var a = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_text");
  a.css({
    "font-family": b.face,
    "font-size": b.size + "px",
    color: $.convertColor(b.color),
...
TYRANO.kag.tmp.backlog.functions.setLine = function () {
  ...
  if (b.img != "") {
    a.css({
      background: "url('" + TYRANO.kag.tmp.backlog.image_path + b.img + "') center center no-repeat"
    })
  } else {
    a.css({
      "background-color": $.convertColor(b.color)
    })
  }
};
TYRANO.kag.tmp.backlog.functions.setVoiceButton = function () {
  ...
  var a = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_voice").find("div");
  a.each(function () {
    $(this).parent().find(".backlog_text").css({
      width: "calc(100% - " + parseInt(TYRANO.kag.tmp.backlog.width_chara) + parseInt(TYRANO.kag.tmp.backlog.width_voice) + "px)"
    });
    if (b.pos == "right") {
      $(this).parent().parent().find(".backlog_voice").insertAfter($(this).parent().parent().find(".backlog_chara"))
    }
...
TYRANO.kag.tmp.backlog.functions.setScrollbar = function () {
  ...
  var f = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_chara");
  var i = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_text");
  var g = 0;
  if (TYRANO.kag.tmp.backlog.voice_flg == "true") {
    var h = $("#backlog").find("#backlog_area").find(".backlog_block").find(".backlog_voice");
    g = h.width()
  }
  c.css({
    "padding-right": parseInt(TYRANO.kag.tmp.backlog.margin_right) + parseInt(d.width) + "px"
  });
  ...
  if (d.tab_img != "") {
    ...
    l.css({
      width: d.width + "px",
      "background-color": "rgba(" + e[0] + "," + e[1] + "," + e[2] + "," + $.convertOpacity(d.opacity) + ")",
